...
`viewmodel.cpp` contains the class used for the view-model capture. The shared `IHelloViewModel` contract in `viewmodel.vm.xml` uses Workflow auto-properties to declare `GetName`/`SetName`, `GetWelcome`, and the corresponding `NameChanged`/`WelcomeChanged` events. `Name` is writable; `Welcome` is read-only. `@rpc:Interface` enables generating the RPC contract, and `@rpc:Ctor` allows clients to request the view model service. These attributes are needed for TypeScript and other implementations outside Workflow or C++. The Instance XML receives this interface as a constructor parameter, binds the textbox into `Name`, and observes `Welcome` on the label.
